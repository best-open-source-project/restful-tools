...
public class ProjectConfigUtil {
    ...
    public static String getApplicationConfig(@NotNull Project project,
        ...
        if (conf == null) {
            Notify.getInstance(project).warning(Bundle.message("notify.error.config.def.notfound", propName));
    ...
    private static PsiFile getConfigurationPsiFile(@NotNull Project project,
        ...
        try {
            Collection<VirtualFile> virtualFiles = FilenameIndex.getVirtualFilesByName(qualifiedName, scope);
            ...
            DumbService.getInstance(project).showDumbModeNotification(Bundle.message(
