...
import com.github.loren.restful.tools.utils.scanner.JaxrsHelper;
import com.github.loren.restful.tools.utils.scanner.SpringHelper;
...
public class RestUtil {
     ...
     */
    @NotNull
    public static String scanListenerProtocol(@NotNull Project project,
            @NotNull GlobalSearchScope scope) {
        // default protocol
        String protocol = "http";

        try {
            String value = ProjectConfigUtil.getApplicationConfig(project, scope,
                    "server.ssl.enabled");
     ...
     */
    @Nullable
    public static PsiAnnotation getClassAnnotation(@NotNull PsiClass psiClass,
     ...
     */
    @NotNull
    public static Set<PsiAnnotation> getMethodAnnotations(@NotNull PsiMethod psiMethod) {
        Set<PsiAnnotation> annotations = new LinkedHashSet<>(
                Arrays.asList(psiMethod.getModifierList().getAnnotations()));
    ...
    @Nullable
    public static PsiAnnotation getQualifiedAnnotation(PsiAnnotation psiAnnotation,
            @NotNull String qualifiedName) {
        ...
        if (targetAnn.equals(annotationQualifiedName) || documentedAnn.equals(
                annotationQualifiedName) || retentionAnn.equals(annotationQualifiedName)) {
            return null;
        }
        ...
        for (PsiAnnotation classAnnotation : psiClass.getAnnotations()) {
            PsiAnnotation qualifiedAnnotation = getQualifiedAnnotation(classAnnotation,
                    qualifiedName);
